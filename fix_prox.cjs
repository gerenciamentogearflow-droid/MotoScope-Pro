...
// It's a JS module exporting `componentsDB`. Let's eval it, modify the object, and write it back.
// Since it exports `componentsDB`, we can replace `export const componentsDB = ` with nothing.
...
if (idx !== -1) {
  ...
  console.log("ignition-coil-secondary-cdi-prox not found in db array");
}
