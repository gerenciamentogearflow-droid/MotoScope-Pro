...
export const deleteUser = async (adminUser: User, usernameToDelete: string): Promise<boolean> => {
  ...
  const cleanToDelete = usernameToDelete ? usernameToDelete.trim() : "";
  if (!cleanToDelete) return false;
  
  if (adminUser.username === cleanToDelete) return false;

  // Deleta localmente
  const localUsers = getLocalUsers();
  const filtered = localUsers.filter((u: any) => u.username !== cleanToDelete);
  saveLocalUsers(filtered);

  // Tenta deletar do Firestore
  try {
    const userDocRef = doc(db, "users", cleanToDelete);
...
export const adminUpdateUser = async (
  ...
  const cleanTarget = targetUsername ? targetUsername.trim() : "";
  const cleanNew = newUsername ? newUsername.trim() : "";
  if (!cleanTarget || !cleanNew) return false;

  const localUsers = getLocalUsers();
  const localIndex = localUsers.findIndex((u: any) => u.username === cleanTarget);
  if (localIndex === -1) return false;

  if (cleanTarget !== cleanNew) {
    if (localUsers.some((u: any) => u.username === cleanNew)) {
  ...
  try {
    const targetDocRef = doc(db, "users", cleanTarget);
    if (cleanTarget !== cleanNew) {
      const newDocRef = doc(db, "users", cleanNew);
...
export const updateUserCredentials = async (
  ...
  const cleanCurrent = currentUser.username ? currentUser.username.trim() : "";
  const cleanNew = newUsername ? newUsername.trim() : "";
  if (!cleanCurrent || !cleanNew) return null;

  const localUsers = getLocalUsers();
  const localIndex = localUsers.findIndex((u: any) => u.username === cleanCurrent);
  ...
  // Tenta atualizar no Firestore
  try {
    const currentDocRef = doc(db, "users", cleanCurrent);
    if (cleanCurrent !== cleanNew) {
      const newDocRef = doc(db, "users", cleanNew);
      await setDoc(newDocRef, updatedData);
  ...
  return { username: cleanNew, role: oldRole };
};
