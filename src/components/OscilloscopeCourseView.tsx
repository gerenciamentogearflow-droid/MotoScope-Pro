...
export function OscilloscopeCourseView({ user, onBack }: OscilloscopeCourseViewProps) {
  ...
  if (showCertificate) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#232F46] flex flex-col p-6 items-center">
        <button
        ...
        <div className="w-full max-w-4xl bg-white dark:bg-[#1A2235] p-12 md:p-24 rounded-none shadow-2xl border-[16px] border-double border-gray-800 relative text-center">
          ...
          <p className="text-3xl md:text-5xl font-bold text-gray-900 dark:text-gray-100 mb-8 border-b-2 border-gray-300 dark:border-[#3D5280] pb-4 inline-block px-12">
            {user.username}
          </p>
  ...
  if (activeQuiz) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#232F46] flex flex-col">
        <header className="px-6 py-4 bg-white dark:bg-[#1A2235] border-b border-gray-200 dark:border-[#2A3B5C] sticky top-0 z-10">
          <div className="max-w-4xl mx-auto w-full flex items-center gap-4">
            <button
              ...
              className="p-2.5 rounded-xl hover:bg-gray-100 dark:bg-[#232F46] text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-gray-100 transition-colors"
            >
        ...
        <main className="flex-1 max-w-3xl mx-auto w-full p-6 py-12">
          <div className="bg-white dark:bg-[#1A2235] rounded-3xl p-8 shadow-sm border border-gray-200 dark:border-[#2A3B5C]">
                    ...
                    {q.options.map((opt, optIdx) => {
                      ...
                      if (!quizSubmitted) {
                        btnClass += isSelected 
                          ...
                          : "border-gray-200 dark:border-[#2A3B5C] hover:border-blue-300 hover:bg-gray-50 dark:bg-[#232F46] text-gray-700 dark:text-gray-300";
                      } else {
                        if (isCorrect) {
                          ...
                          btnClass += "border-gray-200 dark:border-[#2A3B5C] opacity-50";
                        }
                      }
                  ...
                  <div className="flex justify-center gap-4">
                    {!allCorrect ? (
                      <button
                        ...
                        className="px-8 py-3 bg-gray-200 dark:bg-[#232F46] hover:bg-gray-300 text-gray-900 dark:text-gray-100 font-bold rounded-xl transition-colors"
                      >
                        Refazer Prova
                      </button>
                    ) : (
  ...
  if (activeLesson) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#232F46] flex flex-col">
        ...
        <main className="flex-1 max-w-3xl mx-auto w-full p-6 py-12">
          <div className="bg-white dark:bg-[#1A2235] rounded-3xl p-8 md:p-12 shadow-sm border border-gray-200 dark:border-[#2A3B5C]">
    ...
    <div className="pb-12">
      <div className="flex items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
            className="bg-gray-50 dark:bg-[#232F46] hover:bg-gray-100 dark:bg-[#232F46] p-2.5 rounded-xl transition-colors active:scale-95 border border-gray-200 dark:border-[#2A3B5C]/60 shadow-sm"
          >
      ...
      <div className="bg-white dark:bg-[#1A2235] rounded-3xl p-6 border border-gray-200 dark:border-[#2A3B5C] shadow-sm mb-8">
        ...
        <div className="w-full bg-gray-100 dark:bg-[#232F46] h-4 rounded-full overflow-hidden">
          <div 
      ...
      <div className="space-y-6">
        {oscilloscopeCourse.map((module, mIdx) => {
          ...
          return (
          <div key={module.id} className={`bg-white dark:bg-[#1A2235] rounded-3xl border border-gray-200 dark:border-[#2A3B5C] overflow-hidden shadow-sm ${!isUnlocked ? 'opacity-80' : ''}`}>
            <div className="bg-gray-50 dark:bg-[#232F46] border-b border-gray-200 dark:border-[#2A3B5C]">
              {module.imageUrl ? (
            ...
            <div className="divide-y divide-gray-100">
              {module.lessons.map((lesson, lIdx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 sm:p-6 flex items-center justify-between group transition-colors ${isUnlocked ? 'hover:bg-gray-50 dark:bg-[#232F46]' : 'cursor-not-allowed'}`}
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors ${!isUnlocked ? 'bg-gray-100 dark:bg-[#232F46] border-gray-200 dark:border-[#2A3B5C] text-gray-400' : isCompleted ? 'bg-green-100 border-green-500 text-green-600' : 'bg-gray-50 dark:bg-[#232F46] border-gray-300 dark:border-[#3D5280] text-gray-400 group-hover:border-blue-500 group-hover:text-blue-500'}`}>
              ...
              {module.quiz && (
                <button
                  ...
                  className={`w-full text-left p-4 sm:p-6 flex items-center justify-between group transition-colors ${isUnlocked ? 'hover:bg-blue-50/50 bg-slate-50' : 'bg-gray-50 dark:bg-[#232F46] cursor-not-allowed'}`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors ${!isUnlocked ? 'bg-gray-100 dark:bg-[#232F46] border-gray-200 dark:border-[#2A3B5C] text-gray-400' : completedQuizzes.includes(module.quiz.id) ? 'bg-green-100 border-green-500 text-green-600' : 'bg-white dark:bg-[#1A2235] border-blue-200 text-blue-500 group-hover:border-blue-500'}`}>
