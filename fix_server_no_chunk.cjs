...
const newTranslate = `      const prompt = \`Traduza os valores deste JSON para Português do Brasil. Mantenha os termos técnicos. Retorne APENAS um JSON.
      ...
      while (retries > 0) {
        try {
          ...
          if (errorStr.includes("503") || errorStr.includes("high demand") || errorStr.includes("UNAVAILABLE") || errorStr.includes("429") || errorStr.includes("Too Many Requests") || errorStr.includes("quota") || errorStr.includes("RESOURCE_EXHAUSTED")) {
